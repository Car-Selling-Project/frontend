...
const AddOrder = () => {
  ...
  const getOrCreateCustomerId = async (customerInfo) => {
    ...
    try {
      const res = await api.get(`/admins/customers`);
      const found = res.data.customers.find(
        c => c.phoneNumber === customerInfo.phoneNumber
      );
      if (found) return found._id;
      // If not found, create new customer
      const createRes = await api.post(`/admins/customers`, customerInfo);
  ...
  const submitForm = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        customerId: formData.customerId ? customerId : undefined,
        customerInfo: {
