...
export const StarRating = ({ carId }) => {
  ...
  useEffect(() => {
    const fetchAverageRating = async () => {
      try {
        const res = await api.get(`/customers/reviewss/${carId}`); // <-- FIXED ENDPOINT
          ...
          <div className="flex gap-3 mt-2 overflow-x-auto">
            {(car.images || []).map((img, index) => (
              <img
                ...
                src={img}
                alt={`Preview ${index}`}
                className={`w-20 h-14 rounded-md cursor-pointer object-cover transition-transform duration-200 ${selectedImage === index ? "border-2 border-blue-500 scale-105" : ""
                  }`}
